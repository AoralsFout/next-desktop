...
const calculateBarPoints = (angle, barLength) => {
    ...
    switch (alignment.value) {
        case 'top':
            // 顶对齐：频谱条从外圈向内延伸
            startRadius = baseRadius.value + maxBarLength.value
            endRadius = startRadius - barLength
            break
        case 'center':
            // 中心对齐：频谱条从中心向两侧延伸
            startRadius = baseRadius.value - barLength / 2
            endRadius = baseRadius.value + barLength / 2
            break
        case 'bottom':
        default:
            // 底对齐：频谱条从内圈向外延伸（默认）
            startRadius = baseRadius.value
            endRadius = baseRadius.value + barLength
            break
    }
